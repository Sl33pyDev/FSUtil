...
import org.fsp.fsutil.client.renderer.BonnieDisplayItemRenderer;
...
public class SpringBonnieDisplayItem extends BlockItem implements GeoItem {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public SpringBonnieDisplayItem(Block block, Properties settings) {
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        ...
        consumer.accept(new IClientItemExtensions() {
            private final BlockEntityWithoutLevelRenderer renderer = new SpringBonnieDisplayItemRenderer();
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity entity, int slot, boolean selected) {
        if (!stack.hasTag() || !stack.getTag().contains("display")) {
            ...
            display.putString("Name", Component.Serializer.toJson(
                    Component.literal("SpringBonnie").withStyle(style -> style.withColor(TextColor.fromRgb(0xc39860)).withItalic(false))
            ));
